const wordGenerator = () => {
    ...
    let newWordIndex = Math.floor(Math.random() * jokeArray.length);

    return `"${jokeArray[newWordIndex]}"`;
};
...
const adjGenerator = () => {
    ...
    let newAdjIndex = Math.floor(Math.random() * adjectives.length);
    
    return `${adjectives[newAdjIndex].toLocaleUpperCase()}`;
...
const animalGenerator = () => {
    ...
    let newAnimalIndex = Math.floor(Math.random() * animals.length);

    return `${animals[newAnimalIndex].toUpperCase()}`;
}
...
const nounGenerator = () => {
    ...
    let newNounIndex = Math.floor(Math.random() * nouns.length);

    return `${nouns[newNounIndex].toUpperCase()}`;
};
const operation = () => {
  return adjGenerator() + " " + nounGenerator();
};

const codeName = () => {
    return `Welcome Agent ${adjGenerator()} ${animalGenerator()}! You will be assigned to operation ${operation()}!`;
};
